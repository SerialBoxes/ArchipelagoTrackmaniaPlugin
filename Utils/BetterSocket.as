...
class BetterSocketCallback{
    SocketCallback@ cb;
    BetterSocketCallback(SocketCallback@ cb){
        @this.cb = cb;
    }
}
funcdef void SocketCallback();
...
class BetterSocket {
    ...
    void Connect() {
        IsConnecting = true;
        if (s !is null) {
            warn("WARNING: Already connected to a socket");
            IsConnecting = false;
            return;
        }
    ...
    void WriteMsg(uint8 msgType, const string &in msgData) {
        if (s is null) {
            ...
            warn("WriteMsg: dropping msg b/c socket closed/disconnected");
            //return;
        }
        bool success = true;
        ...
        if (!success) {
            warn("failure to write message?" + " / " + msgData.Length + " bytes");
            // this.Shutdown();
        }
...
class RawMessage {
    ...
    void ReadFromSocket(Net::Socket@ s, uint len) {
        ...
        print ("message length: readStrLen");
        print ("message: " + msgData);
        // string msgTypeStr = tostring(TCPMessageTypes(msgType));
